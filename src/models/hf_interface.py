...
class HFInterface(LLMInterface):
    ...
    def __init__(self, model_name: str, model_kwargs: Dict[str, Any] = {}, **kwargs):
        ...
        if self.verbose:
            print(
                f"Initializing HFInterface for {self.model_name}"
            )  # Conditional print
        ...
        if requested_device == "auto":
            if torch.cuda.is_available():
                ...
                try:
                    ...
                    print(
                        "Warning: MPS available but may not be functional. Falling back to CPU."
                    )
        ...
        if self.verbose:
            print(f"Using device: {self.device}")  # Conditional print
        ...
        try:
            ...
            if self.tokenizer.pad_token is None:
                ...
                # Keep warning messages
                print(
                    f"Warning: Tokenizer for {model_name} lacks a pad token. Using EOS token ({self.tokenizer.eos_token}) for padding."
                )
        except Exception as e:
            print(
                f"Error loading tokenizer for {model_name}: {e}"
            )  # Keep error messages
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"Attempting to load model with identifier: '{model_name}'"
                )  # Conditional print
            # ---------------------------------
            ...
            print(f"Error loading model {model_name}: {e}")  # Keep error messages
    ...
    def compute_logprobs(
        ...
        Verbosity is controlled by the 'verbose' flag set during initialization.
        ...
        if self.verbose:
            print(
                f"Using batch size: {batch_size}, temperature: {temperature} (from init params)"
            )  # Conditional print
        ...
        if not isinstance(temperature, (float, int)) or temperature <= 0:
            # Keep warning messages
            print(
                f"Warning: Invalid temperature value ({temperature}). Must be a positive number. Using default temperature 1.0."
            )
            temperature = 1.0
        ...
        if self.verbose:
            print(
                f"Preparing {len(prompt_data)} prompts for HF batching..."
            )  # Conditional print
        start_time = time.time()
        lengths_and_data = []
        for i, data_tuple in enumerate(prompt_data):
            prompt, expected, _, metadata_key = (
                data_tuple[1],
                data_tuple[2],
                data_tuple[3],
                data_tuple[4],
            ...
            try:
                ...
                # Keep warning messages
                print(
                    f"Warning: Error tokenizing item for length calculation (key: {metadata_key}): {e}. Skipping."
                )
        ...
        if self.verbose:
            print(f"Sorting completed in {sort_time:.2f} seconds.")  # Conditional print

        # --- 2. Process in Batches ---
        num_batches = math.ceil(len(sorted_prompt_data) / batch_size)
        if self.verbose:
            print(
                f"Processing in {num_batches} batches of size {batch_size}..."
            )  # Conditional print

        for i in tqdm(
            range(0, len(sorted_prompt_data), batch_size),
            ...
            try:
                ...
                prompt_only_tokens = self.tokenizer(
                    batch_prompts, padding=False, truncation=True
                )["input_ids"]
                ...
                for batch_idx in range(len(batch_data)):
                    ...
                    end_idx = (
                        inputs["attention_mask"][batch_idx].sum().item()
                    )  # Actual sequence length without padding

                    # Ensure start_idx is not beyond the actual sequence length
                    if start_idx >= end_idx:
                        # Keep warning messages
                        print(
                            f"Warning: Prompt length ({prompt_len}) is >= sequence length ({end_idx}) for key {metadata_key}. Expected response might be empty or truncated. Setting logprob to 0.0."
                        )
                    ...
                    token_logprobs = log_probs_full[
                        batch_idx, start_idx - 1 : end_idx - 1, :
                    ]  # Shape: (resp_len, vocab_size)

                    # Gather the log probabilities of the actual response tokens
                    gathered_logprobs = torch.gather(
                        token_logprobs, 1, response_token_ids.unsqueeze(-1)
                    ).squeeze(-1)
                    ...
                    if math.isnan(total_logprob):
                        # Keep warning messages
                        print(
                            f"Warning: NaN log probability calculated for key {metadata_key}. Setting to -inf."
                        )
                        current_batch_results[metadata_key] = (
                            -float("inf"),
                            num_response_tokens,
                ...
                for key in batch_metadata_keys:
                    if (
                        key not in current_batch_results
                    ):  # Avoid overwriting results from previous successful batches
        ...
        if self.verbose:
            print(
                f"Finished processing all batches for {self.model_name}."
            )  # Conditional print
        return results  # type: ignore

    def release(self):
        """Releases GPU memory used by the model and tokenizer."""
        # Use self.verbose which was set in __init__
        if self.verbose:
            print(
                f"Releasing HF model and tokenizer for {self.model_name} from {self.device}..."
            )  # Conditional print
        del self.model
        del self.tokenizer
        if self.device == torch.device("cuda"):
            ...
            pass
        if self.verbose:
            print("HF resources released.")  # Conditional print